...
/**
 * Dropdown Portal Component for rendering dropdown outside of normal DOM hierarchy
 */
...
const DropdownPortal: React.FC<DropdownPortalProps> = ({ isOpen, onClose, buttonRef }) => {
  ...
  useEffect(() => {
    if (isOpen && buttonRef.current && mounted) {
      ...
      // Get responsive left offset based on screen size
      let leftOffset = 224; // Default for lg+ screens
      if (window.innerWidth < 768) {
        leftOffset = 90; // Mobile
      } else if (window.innerWidth < 1024) {
        leftOffset = 160; // md screens
      }
      
      setPosition({
        top: rect.bottom + 8,
        left: rect.left - leftOffset + rect.width
      });
    }
  ...
  return createPortal(
    <>
      ...
      {/* Dropdown menu */}
      <div 
        className="fixed w-56 bg-white rounded-lg shadow-2xl border border-gray-200 py-2 z-[9999] animate-fade-in"
        style={{ 
          top: position.top,
          left: position.left,
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={cn("bg-white border-b border-gray-200 sticky top-0 z-[999]", className)}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="btn-base p-2 sm:hidden text-gray-700 hover:text-gray-900 hover:bg-gray-100 border border-gray-300 hover:border-gray-400"
            ...
            {/* Notifications */}
            <button title="پیام ها" className="btn-base btn-sm p-2 relative text-gray-700 hover:text-gray-900 hover:bg-gray-100 border border-gray-300 hover:border-gray-400">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
